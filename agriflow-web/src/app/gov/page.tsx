  ...
  loading: () => (
    <div className="w-full h-full bg-[#0A0D14] flex items-center justify-center rounded-3xl border border-white/5">
      <div className="text-center">
        ...
        <p className="text-[10px] font-bold text-white/40 uppercase tracking-widest">Initialising National Intelligence…</p>
      </div>
    </div>
  ),
});
...
const STRATEGIC_NODES = [
  { id: 1, position: [-6.2088, 106.8456] as [number, number], title: "Pusat Distribusi Nasional", status: "Normal" },
  { id: 2, position: [-7.2575, 112.7520] as [number, number], title: "Lumbung Pangan Timur", status: "Tinggi" },
  { id: 3, position: [-6.8726, 109.0317] as [number, number], title: "Kluster Bawang Merah", status: "Normal" },
  { id: 4, position: [3.5952, 98.6722] as [number, number], title: "Distribusi Sumatera", status: "Kritis" },
];
...
export default function GovDashboard() {
  ...
  const markers = useMemo(() => {
    return STRATEGIC_NODES.map(node => ({
      ...node,
      content: (
        <div className="mt-1 text-[10px] text-slate-500 italic">
          Kapasitas: 85% • Status: {node.status}
        </div>
      )
    }));
  }, []);
  ...
  return (
    <DashboardLayout>
      <div className="p-8 max-w-7xl mx-auto space-y-12 animate-in fade-in duration-700 selection:bg-[#14b850]/30 selection:text-white">
        ...
        <div className="bg-[#0A0D14] p-10 rounded-[48px] flex flex-col md:flex-row justify-between items-start md:items-center gap-8 shadow-2xl border border-white/[0.05] relative overflow-hidden group">
          ...
          <div className="flex items-center space-x-6 text-white relative z-10">
            <div className="w-16 h-16 bg-[#14b850] rounded-2xl flex items-center justify-center shadow-[0_0_20px_rgba(20,184,80,0.4)] transform group-hover:rotate-6 transition-transform duration-500">
               <span className="text-[#0A0D14] font-black text-3xl tracking-tighter font-outfit">G</span>
            </div>
            <div>
              <h1 className="font-bold text-3xl md:text-4xl leading-none tracking-tight mb-2 text-white">National Food <span className="text-[#14b850]">Security</span></h1>
              <p className="text-[10px] uppercase tracking-[0.3em] text-[#14b850] font-bold italic">Dashboard Penelitian Ketahanan Pangan Nasional</p>
            </div>
          </div>
          <div className="flex items-center space-x-6 text-white relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          <RiskIndicator label="National Risk" status={riskData?.risk || "LOW"} color="text-[#f59e0b]" bgColor="bg-[#f59e0b]/10" borderColor="border-[#f59e0b]/20" icon={Warning} />
          <RiskIndicator label="Food Inflation" status={riskData?.inflation || "2.4%"} color="text-[#f43f5e]" bgColor="bg-[#f43f5e]/10" borderColor="border-[#f43f5e]/20" icon={ChartLineUp} />
          <RiskIndicator label="Subsidy Realization" status={riskData?.subsidy || "84%"} color="text-[#14b850]" bgColor="bg-[#14b850]/10" borderColor="border-[#14b850]/20" icon={ShieldCheck} />
          <RiskIndicator label="Grain Stocks" status={riskData?.stocks || "12.4M T"} color="text-[#0ea5e9]" bgColor="bg-[#0ea5e9]/10" borderColor="border-[#0ea5e9]/20" icon={MapTrifold} />
        </div>

        {/* National Strategic Map - NEW FOR RESEARCH */}
        <div className="bg-[#0A0D14] p-10 rounded-[48px] border border-white/[0.05] shadow-2xl relative group overflow-hidden">
           <div className="flex justify-between items-center mb-10 relative z-10">
              <div>
                 <h2 className="text-2xl font-bold text-white tracking-tight">National Strategic Intelligence Map</h2>
                 <p className="text-[10px] text-[#14b850] font-bold uppercase tracking-widest mt-1">Pemetaan Titik Rawan & Cadangan Pangan Strategis</p>
              </div>
              <div className="bg-[#14b850]/10 px-6 py-2.5 rounded-2xl border border-[#14b850]/20 text-[#14b850] text-[10px] font-bold uppercase tracking-widest animate-pulse">
                 Live Monitoring Active
              </div>
           </div>
           <div className="h-[500px] rounded-[32px] overflow-hidden border border-white/5 shadow-inner">
              <LeafletMap center={[-0.7893, 113.9213]} zoom={5} markers={markers} />
           </div>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2 space-y-10">
            <div className="bg-white/[0.02] backdrop-blur-3xl p-10 rounded-[48px] border border-white/[0.05] shadow-xl">
               <div className="flex justify-between items-center mb-10">
                  <h2 className="text-xl font-bold text-white tracking-tight">Proyeksi Inflasi Komoditas Strategis</h2>
            ...
            <div className="bg-white/[0.02] backdrop-blur-3xl p-10 rounded-[48px] border border-white/[0.05] group shadow-xl">
              <div className="flex justify-between items-center mb-10">
                 <h2 className="text-xl font-bold text-white tracking-tight">Subsidy Gap Analysis (Regional)</h2>
          ...
          <div className="space-y-10">
            <div className="bg-white/[0.02] backdrop-blur-3xl p-10 rounded-[48px] border border-white/[0.05] shadow-xl">
              <div className="flex items-center space-x-4 mb-10">
                ...
                <h2 className="text-xl font-bold text-white tracking-tight">Early Warning</h2>
              </div>
            ...
            <button 
              ...
              className="w-full bg-[#14b850] text-[#0A0D14] py-6 rounded-[32px] font-bold text-[11px] uppercase tracking-widest shadow-[0_20px_40px_rgba(20,184,80,0.2)] hover:shadow-[0_25px_50px_rgba(20,184,80,0.4)] hover:-translate-y-1 transition-all flex items-center justify-center space-x-3 group"
            >
              <FilePdf size={24} weight="fill" className="group-hover:scale-110 transition-transform" />
              <span>Export Laporan Riset (PDF)</span>
            </button>
          </div>
        </div>
      </div>
...
function RiskIndicator({ label, status, color, bgColor, borderColor, icon: Icon }: RiskIndicatorProps) {
  return (
    <div className={`bg-[#0A0D14] p-10 rounded-[48px] border border-white/[0.05] flex items-center space-x-6 shadow-xl hover:border-white/[0.1] transition-all cursor-default group transform hover:-translate-y-1 duration-500`}>
      ...
      <div>
        ...
        <p className={`text-2xl font-bold ${color} tracking-tight`}>{status}</p>
      </div>
    </div>
  );
}
...
function SubsidyRow({ region, actual, need, status }: SubsidyRowProps) {
  ...
  return (
    <div className="flex items-center justify-between p-6 bg-[#0A0D14] rounded-3xl border border-white/[0.05] hover:border-white/[0.15] hover:bg-white/[0.02] transition-all group cursor-pointer shadow-sm">
      <div className="w-1/3">
        <p className="font-bold text-white text-lg group-hover:text-[#14b850] transition-colors">{region}</p>
      </div>
...
function AlertItem({ severity, title, desc }: AlertItemProps) {
  ...
  return (
    <div className={`p-6 rounded-3xl border border-white/[0.05] relative overflow-hidden bg-[#0A0D14] hover:bg-white/[0.02] hover:border-white/[0.15] transition-all group cursor-pointer`}>
      ...
      <div className="flex justify-between items-start mb-2 pl-4">
         <h4 className="font-bold text-white tracking-tight text-sm group-hover:text-[#14b850] transition-colors">{title}</h4>
         <Globe size={18} className="text-[#14b850] opacity-0 group-hover:opacity-100 transition-opacity" />
      </div>
